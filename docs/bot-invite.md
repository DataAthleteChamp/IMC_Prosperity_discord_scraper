# Pitch template: asking IMC to invite a read-only bot

The Terms-of-Service–clean path for this scraper is a **Bot account** invited
to the Discord server by an admin. If you can persuade IMC (or whoever runs
the Prosperity Discord) to invite your bot, you get:
...
5. Save the token to `.env` as `DISCORD_BOT_TOKEN`.

## Email/DM template

> Hi team,
>
> I'm a Prosperity participant building an open-source, **read-only** archive
...
> `https://github.com/DataAthleteChamp/IMC_Prosperity_discord_scraper`.
>
...
Fall back to the user-token backend (`--auth user`). Read the ToS warning in
the README first.
